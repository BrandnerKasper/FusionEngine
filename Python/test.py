...
from utility import tensor_to_ascii
...
def test() -> None:
    ...
    test_dataset = ASCIISnake("data/test", 100)
    ...
    for in_txt, gt_img in test_loader:
        ...
        with torch.inference_mode():
            pred = model(in_txt)
        display(in_txt, pred, gt_img)
        print("Test!")


def display(txt_t: torch.Tensor, pred_t: torch.Tensor, gt_t: torch.Tensor) -> None:
    ascii_str = tensor_to_ascii(txt_t)
